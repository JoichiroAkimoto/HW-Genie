...
- `docs/assets/hw-genie-demo.gif` — `vhs` で生成された GIF（約 2.4 MB 圧縮後 / 3.9 MB 生、1200×1050, 約 33 秒, Dracula テーマ）
...
1. `uv run hw-genie auth --list` — 4 アカウント表（Arthur / Morgana / Elyndra / Kaito。Arena 1位を含む黄色ハイライト、Energy 212/190 over cap は赤、Updated は `2026-08-24 10:27:48` のフル形式）
2. ↓ 改行3回で間を空け → `uv run hw-genie daily --account Arthur` — Hero Raid 3件 Skipping + 4件実行 → Exchanging Soul Stones → Item Raid 2回 → Soul Shop Slot 2-6 購入 5件 → Account Status（212/190 → 42/190 に減少）→ 10028 Titan Artifact 実行 → 完了。入力時は `zsh-autosuggestions` によるグレー ghost 補完が表示されます
