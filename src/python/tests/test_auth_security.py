...
def test_db_connection_error_handling():
    ...
    with patch("hw_genie.core.repository.SessionLocal") as mock_session_local:
        # SessionLocal() が呼ばれたときに例外を投げるように設定
        mock_session_local.return_value.__enter__.side_effect = SQLAlchemyError("DB connection failed")
