...
- **ギルドクエスト（「Obtain xxx Sparks of Power」等、2000xxxx/2001xxxx ファミリ）**: ID が日次・アカウントで動的なため `QUEST_OPERATIONS` の固定 ID 登録ではなく `quest_guild_defaults`（config キー `quest_guild_defaults`）で制御します。`--init-defaults` で `enabled:false`＋`heroId`（既定 38）を自動投入し、`--set-default guild enabled true` で有効化します:
    ...
    ```
    claimable（state=2）のギルドクエスト報酬受領は設定に関係なく常時実行されますが、**自動受領しない報酬があります**（コード側 `GUILD_QUEST_CLAIM_EXCLUDE`）: 20010002（stamina 200 = エナジー回復）、20010003（オラクルカード）、20010004（SOUL クリスタル）、20010005（ポータル）は questFarm による claim 対象から除外され、実行時は `Skipping claim for 20010002 ...` と表示されます（dry-run では `SKIP (GUILD_QUEST_CLAIM_EXCLUDE)`）。除外対象のギルドクエストは未受領のまま残ります。進行中（state=1）のものは有効時のみ heroTitanGift レシピ（10023 と同一: LevelUp ×2 → Drop）を実行して Sparks of Power を獲得、その後 questGetAll を取り直して state=2 になったものをまとめて受領します。レシピは**1 ゲームデイ（userGetInfo の nextDayTs から求めた今日のサイクル開始時刻ベース）に 1 セットのみ**実行されます（進捗には依存しない時刻ベースの 1 日 1 回ガード。ギルドクエストはギルド全体の累積ポイントで達成するため、1 セット実行後に達成分を受領するだけで繰り返し実行はしません）。実行記録は `last_recipe_at`（最後にレシピを実行した Unix 秒）に保存され、これが今日のサイクル開始時刻（nextDayTs − 86400）以上なら「今日は実行済み」とみなしてスキップされます（nextDayTs が取れない環境ではガード無効＝毎回実行）。レシピ失敗（資源不足等）時は `last_recipe_at` を記録しないため、次の実行機会に再試行できます（成功済みステップからではなく 1 セット全体を再実行するため Gift 資源を再消費します）。デイリー 10023（同一レシピ）の成功も「今日の実行済み」として `last_recipe_at` に記録されます（Gift 資源の二重消費防止）。なお 10023 は達成条件が小さく 1 回目の LevelUp で達成（claimable）になった場合でも、Sparks 稼ぎのレシピ全体（LevelUp ×2 → Drop）を中断せず最後まで実行します（報酬 claim は達成検知時に 1 回のみ。1 ステップだけで終えて「実行済み扱い」になるのを防ぐため）。旧方式（`recipe_runs` / `max_recipes`）で管理していたアカウントは `last_recipe_at` 未設定のため移行初日のみ 1 回追加で実行されます（以後は 1 日 1 回に収束）。ギルドクエストが 0 件の日（月 1 回程度）は自動スキップされます。`multi quests` / `multi daily` / `multi full` にも同じロジックが組み込まれています。
